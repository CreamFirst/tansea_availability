...
app.get("/", (req, res) => {
 res.send("Tansea availability service running");
});
...
if (interpretation.kind === "vagueRange") {
 ...
 const first = weeks[0];
 const priceText = first.price !== null ? `around £${first.price}` : "available";

 // Build a full summary list of up to 3 weeks
 let summaryList = weeks
   .slice(0, 3)
   .map((w) => {
 ...
 // Remove the FIRST week to avoid duplication
 summaryList = summaryList.split("; ").slice(1).join("; ");
