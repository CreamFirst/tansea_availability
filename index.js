import express from "express";
import cors from "cors";
import ical from "node-ical";
import fs from "fs";

const app = express();
app.use(cors());
app.use(express.json());

// ---- Load pricing table ----
const prices = JSON.parse(fs.readFileSync("./prices.json", "utf8"));

// ---- iCal feed URL ----
const ICAL_URL =
 "https://api.bookalet.co.uk/v1/16295/bookalet-723489/26085.ics";

// Convert a date string to a numeric timestamp
function toNum(d) {
 return new Date(d).getTime();
}

// Find weekly price band for a given date
function getPriceForDate(dateStr) {
 const target = toNum(dateStr);

 for (const p of prices) {
   const start = toNum(p.start);
   const end = toNum(p.end);

   if (target >= start && target < end) {
     return p.price;
   }
 }
 return null; // no price found
}

// Load bookings from the iCal feed
async function loadBookings() {
 const data = await ical.async.fromURL(ICAL_URL);
 let bookings = [];

 for (let ev of Object.values(data)) {
   if (ev.type === "VEVENT") {
     bookings.push({
       start: ev.start,
       end: ev.end,
     });
   }
 }

 return bookings;
}

// Check if a single date is booked
function isBooked(dateStr, bookings) {
 const d = new Date(dateStr);
 return bookings.some((b) => d >= b.start && d < b.end);
}

// ------------------------------
// MAIN /check ENDPOINT (simple)
// ------------------------------
app.post("/check", async (req, res) => {
 try {
   const { date } = req.body;

   if (!date) {
     return res.status(400).json({ error: "Missing date" });
   }

   const bookings = await loadBookings();

   const booked = isBooked(date, bookings);
   const price = getPriceForDate(date);

   // Build a friendly message
   let message;
   if (booked) {
     message = "Sorry — that date is booked.";
   } else if (price !== null) {
     message = `Great news — that week is £${price}. Short stays available on request.`;
   } else {
     message = "That date appears available. Short stays available on request.";
   }

   return res.json({
     date,
     booked,
     price,
     message,
   });
 } catch (err) {
   console.error("ERROR in /check:", err);
   return res.status(500).json({ error: "Server error" });
 }
});

// Root route (for quick sanity check)
app.get("/", (req, res) => {
 res.json({ status: "Tansea availability API is running" });
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () =>
 console.log("Tansea Availability API running on " + PORT)
);

