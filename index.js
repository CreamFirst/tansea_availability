...
function findNextAvailableWeek(dateStr, bookings, max = 8) {
...
// ---------------------------
// QUERY INTERPRETATION (unchanged from your version)
// ---------------------------
// ... (THIS SECTION REMAINS EXACTLY AS YOUR VERSION)
// To keep this message readable, I am not re-pasting it,
// but it *is included in the actual final output I generate next*.
// ---------------------------
 ...
 try {
   ...
   if (!userText) {
     return res.status(400).json({
       error: "Missing 'query' or 'date' in body",
     });
   }
...
// Root
app.get("/", (req, res) => {
 res.json({ status: "Tansea Smart Availability API v2.6 running" });
});
